...
maintenant facilement voir que les différentes lignes peuvent être calculés en parallele.
...
Pour \emph{ssandPile\_compute\_omp\_taskloop()} on a dupliquer la version tile pour y ajouter
un \em{pragam omp single}, pour que un thread definisse les taches, et un \em{pragam omp task}
devant l'appelle de la fonction \em{do\_tile()} pour que le calcule des tuiles ce trouve dans un thread.\\

On vois que taskloop avec des tuile de taille 64 et 32 est presque aussi bonne que omp\_tile avec
des tuile de taille 64, 32 et 16. Et que taskloop nest pas trés efficace avec des tuille de taille 8 ou 16.
\begin{figure}[H]
    \centering
...
Pour paralleliser avec \emph{asandPile\_compute\_omp\_tiled}, il a fallu créer 4 nids de boucles afin d'éviter
les lectures/écritures concurentes.
...
Le gain de performance par rapport au séquentiel avec tuiles optimisées est de $1.98$ car
$\frac{30927.379}{15641.740}$
